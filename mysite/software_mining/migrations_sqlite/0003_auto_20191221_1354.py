# Generated by Django 3.0.1 on 2019-12-21 13:54
...
def sync_relation(apps, schema_editor):
    ...
    index = 0
    for project_name in projects[index:300]:
        index += 1
        print('\nProject %s: %s' % (index, project_name))
        if project_name.startswith('.') or project_name.endswith('.csv') or project_name.endswith('.txt'):
            continue
        processor.write_relation_to_db(project_name, 'train_set.txt')


class Migration(migrations.Migration):

    dependencies = [
        ('software_mining', '0002_auto_20191221_1239'),
    ]

    operations = [
        migrations.RunPython(sync_relation, hints={'target_db': 'default'}),
    ]
